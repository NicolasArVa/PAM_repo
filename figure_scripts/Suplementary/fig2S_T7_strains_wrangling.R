...
growth_burden_GFP <-tidy_DE3_Hlim %>%
  mutate(iptg=factor(iptg), 
         strain=factor(strain,c("MG1655","MG1655 lacI","BLR","BLR lacI","BL21 lacI")))%>%
  filter(iptg %in% c(1,n), time <= 5)%>%
  ...
  facet_wrap(~strain,ncol = 2, as.table = F)+
...
labels<-data.frame(x = 0,y = 1, label = rev(LETTERS[1:5]),strain=factor(c("MG1655 lacI","MG1655","BLR lacI","BLR","BL21 lacI")))
